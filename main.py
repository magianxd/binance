    ...
    def monitor(self):
        ...
        while self.symbols:
            ...
            if sold_asset and sold_asset['sold'] and sold_asset['buy_timestamp'] != timestamp:
                ...
                self.log('Total earnings so far: {}'.format(self.total_eth_earning))
                self.assets.remove(sold_asset)

            price_open = float(kline[1])
            price_now = float(kline[4])

            percent_increased = price_now / price_open - 1

            if self.trigger_percent <= percent_increased < self.trigger_percent * 1.5:
                ...
                eth_used = float(eth_asset['free']) / 2

                if float(symbol['filters'][0]['minPrice']) <= eth_used <= float(symbol['filters'][0]['maxPrice']):
                    quantity = float(eth_used) / price_now
        ...
        while True:
            ...
            if not asset['profit_low_taken'] and price_now / asset['buy_price'] - 1 >= self.take_profit_low_percent:
                ...
                if order_info:
                    ...
                    asset['eth_earning'] += sell_price * quantity
                    asset['quantity'] -= quantity
                    asset['profit_low_taken'] = True
                    with self.lock:
                        self.total_eth_earning += asset['eth_earning']
                    self.log(asset['name'], 'Sell {} at price {} ETH, total earning: {} ETH'.format(
                        quantity, sell_price, self.total_eth_earning))
